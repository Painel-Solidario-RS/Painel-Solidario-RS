...
import { Box, Checkbox, FormControl, FormLabel, SelectChangeEvent } from "@mui/material";
...
function Shelter() {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const tips: ItemsProps[] = response.data.map((nome: string, index: number) => ({
          key: index,
          value: nome,
        }));
        setPlaceTips(tips);
  ...
  const handleChangePlace = (event: SelectChangeEvent<string>) => {
    ...
    const selectedObject = placeTips.find((item) => item.value === selectedPlaceValue);
    if (selectedObject) {
  ...
  return (
    <BasePage leftIcon title="Cadastro de Abrigo">
      ...
      >
        <Input label="Nome" placeholder="Nome e Sobrenome" sxInput={stylesInput} />
        ...
        <Box sx={{ flex: 1 }}>
          ...
          >
            <ButtonForm content="Animais" icon={<Checkbox checked={true} onChange={() => ""} />} />
            <ButtonForm content="Pessoas" icon={<Checkbox checked={true} onChange={() => ""} />} />
          </FormControl>
        </Box>
        {/* publico alvo */}
        <SelectInput
        ...
        {/* <Select/> */}
        {/* Telefone (Whatsapp) */}
        <Input label="Telefone (Whatsapp)" placeholder="(00) 00000-0000" sxInput={stylesInput} />
      </Box>
      ...
      >
        ...
        />
        {/* Endereço (logradouro) */}
        <Input label="Endereço (logradouro)" placeholder="Ex. 7100000" sxInput={stylesInput} />
        {/* Número */}
        <Input label="Número" placeholder="Informe o número" sxInput={stylesInput} />
        {/* Complemento */}
        <Input label="Complemento" placeholder="Complemento" sxInput={stylesInput} />
        {/* Bairro */}
      ...
      >
        {/* Capacidade Total */}
        <Input label="Capacidade Total" placeholder="Quantidade de pessoas" sxInput={stylesInput} />
